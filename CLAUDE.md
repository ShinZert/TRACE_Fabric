...
- **`services/schema_validator.py`** — Two-pass validation: jsonschema against the Fabric schema, then semantic checks (≥1 finalOutcome, ≥1 entry node — i.e. at least one element with no incoming flow; multiple entries are allowed, only a pure cycle with zero entries fails — no dead ends, valid flow refs, no duplicate IDs, finalOutcomes have no outgoing flows). Fabric has no separate start-event type; entries are identified structurally.
